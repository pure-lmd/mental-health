<template>
  <div class="publicity-manage">
    <div class="page-title"><svg-icon name="publicity" :size="24" /><span>心理宣传管理</span></div>
    <div class="search-form">
      ...
      <el-button type="primary" @click="handleAdd"><svg-icon name="add" :size="14" /><span>添加宣传</span></el-button>
    </div>
...
import { getPublicityList, addPublicity, updatePublicity, deletePublicity, updatePublicityStatus } from '@/api/admin/publicity'
...
const dialogTitle = computed(() => formData.id ? '编辑宣传' : '添加宣传')
const uploadUrl = '/api/file/upload', uploadHeaders = computed(() => ({ Authorization: `Bearer ${getToken()}` }))

async function fetchList() { loading.value = true; try { const res = await getPublicityList(queryParams); tableData.value = res.data.records; total.value = res.data.total } catch (e) { console.error(e) } finally { loading.value = false } }
...
function handleDelete(row) { ElMessageBox.confirm('确定要删除吗？', '提示', { type: 'warning' }).then(async () => { await deletePublicity(row.id); ElMessage.success('删除成功'); fetchList() }).catch(() => {}) }
async function handleStatusChange(row) { try { await updatePublicityStatus(row.id, row.status); ElMessage.success('状态更新成功') } catch (e) { row.status = row.status === 1 ? 0 : 1 } }
...
async function handleSubmit() { const valid = await formRef.value.validate().catch(() => false); if (!valid) return; submitLoading.value = true; try { if (formData.id) { await updatePublicity(formData); ElMessage.success('更新成功') } else { await addPublicity(formData); ElMessage.success('添加成功') }; dialogVisible.value = false; fetchList() } finally { submitLoading.value = false } }
...
.publicity-manage { .search-form { display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: $spacing-md; }
