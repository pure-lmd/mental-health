<template>
  <div class="news-detail">
    <div class="breadcrumb-wrapper"><el-breadcrumb><el-breadcrumb-item :to="{ path: '/front/home' }">首页</el-breadcrumb-item><el-breadcrumb-item :to="{ path: '/front/news' }">文化资讯</el-breadcrumb-item><el-breadcrumb-item>{{ detail.title }}</el-breadcrumb-item></el-breadcrumb></div>
...
import { getNewsDetail } from '@/api/front/news'
...
async function fetchDetail() { loading.value = true; try { const res = await getNewsDetail(route.params.id); detail.value = res.data } catch (e) { console.error(e) } finally { loading.value = false } }
onMounted(() => { fetchDetail() })
</script>

<style lang="scss" scoped>
.news-detail { .content-wrapper { background: $bg-white; border-radius: $border-radius-base; padding: $spacing-xl; box-shadow: $shadow-light; min-height: 500px; }
